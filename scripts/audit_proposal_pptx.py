...
def python_pptx_blank_frames(deck_path: Path, slide_count: int, report: Report):
    """When python-pptx is available, detect oversized empty shapes."""
    try:
    ...
    for idx, slide in enumerate(prs.slides, start=1):
        ...
        for shape in slide.shapes:
            area_ratio = (shape.width or 0) * (shape.height or 0) / (slide_w * (prs.slide_height or 1))
...
def audit(deck_path: Path, script_path: Path | None, report: Report, template_mode: bool = False):
    ...
    if slide_count:
        python_pptx_blank_frames(deck_path, slide_count, report)
